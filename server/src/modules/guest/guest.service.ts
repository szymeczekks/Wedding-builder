...
export const guestService = {
    createGuest: async ({ guestListId, projectId, name, type }: { guestListId: string, projectId: string, name: string, type: string }): Promise<IGuest> => {
        ...
        const guest = Guest.create({ guestListId, projectId, name, type });
        return guest;
    },
    updateGuest: async ({ id, updateInput }: { id: string, updateInput: IGuestUpdateInput }): Promise<IGuest | null> => {
        ...
        const ALLOWED_FIELDS = ['name', 'type'];
